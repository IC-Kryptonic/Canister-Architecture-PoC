...
class TestVideoInfo extends React.Component {
  ...
  async componentDidMount() {
    let res = await videoBackend.getDefaultFeed(10);
    let feed = res.map((video) => <li>{video.name}</li>);
  ...
  async doSearch() {
    ...
    this.setState({
      ...this.state,
      message: videoInfo.name + ': ' + videoInfo.description,
    });
  ...
  async doUpload() {
    ...
    const videoBuffer =
      (await this.state.file?.arrayBuffer()) || new ArrayBuffer(0);
